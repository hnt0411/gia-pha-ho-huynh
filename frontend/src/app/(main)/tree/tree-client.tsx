                    ...
                    {loading ? (
                        ...
                        <div style={{
                            ...
                            {visibleNodes.map(item => {
                                const childOfFamily = treeData?.families.find(f => f.children.includes(item.node.handle));
                                const birthOrder = childOfFamily ? childOfFamily.children.indexOf(item.node.handle) + 1 : 0;
...
const MemoPersonCard = memo(PersonCard, (prev, next) =>
    prev.item === next.item &&
    prev.birthOrder === next.birthOrder &&
        ...
        >
            <div className="px-2.5 py-2 h-full flex items-center gap-2.5">
                ...
                <div className="relative flex-shrink-0">
                    {birthOrder > 0 && (
                        <span className="absolute -top-1 -left-1 w-4 h-4 rounded-full bg-slate-600 text-white
                            text-[8px] flex items-center justify-center shadow-sm font-bold ring-1 ring-white z-10">
                            {birthOrder}
                        </span>
                    )}
